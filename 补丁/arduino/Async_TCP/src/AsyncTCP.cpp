...
void AsyncServer::begin() {
  ...
  int8_t err;
  {
    ...
    if (_pcb) {
#if SO_REUSE
      // Allow the listening port to be re-bound right after end(): lwIP only
      // permits reusing a port when BOTH the new pcb and the conflicting pcb
      // carry SOF_REUSEADDR. Accepted connections inherit this flag from the
      // listener via SOF_INHERITED, so their TIME_WAIT sockets no longer make
      // a subsequent tcp_bind() fail with ERR_USE (-8).
      _pcb->so_options |= SOF_REUSEADDR;
#endif
    }
  }
  if (!_pcb) {
