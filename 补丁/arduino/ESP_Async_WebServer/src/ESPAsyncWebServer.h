...
protected:
  ...
  // Live client requests, tracked so end() can tear down their connections
  // (RST) before begin() is called again.
  mutable asyncsrv::mutex_type _clients_mutex;
  std::list<std::weak_ptr<AsyncWebServerRequest>> _clients;
  bool _closing;

  void _registerClient(const std::shared_ptr<AsyncWebServerRequest> &client);
  void _teardownClients();
