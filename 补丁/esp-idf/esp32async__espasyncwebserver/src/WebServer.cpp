...
AsyncWebServer::AsyncWebServer(uint16_t port) : _server(port), _closing(false) {
  ...
  _server.onClient(
    [](void *s, AsyncClient *c) {
      ...
      c->setRxTimeout(3);
      AsyncWebServer *server = static_cast<AsyncWebServer *>(s);
      std::shared_ptr<AsyncWebServerRequest> r = AsyncWebServerRequest::create(server, c);
      if (!r) {
        c->abort();
        delete c;
        return;
      }
      server->_registerClient(r);
    },
    this
  );
}
...
void AsyncWebServer::begin() {
  {
    asyncsrv::lock_guard_type lock(_clients_mutex);
    _closing = false;
  }
...
void AsyncWebServer::end() {
  _server.end();  // stop accepting new connections first
  _teardownClients();
}

void AsyncWebServer::_registerClient(const std::shared_ptr<AsyncWebServerRequest> &client) {
  asyncsrv::lock_guard_type lock(_clients_mutex);

  // Drop entries whose connections have already closed on their own.
  _clients.remove_if([](const std::weak_ptr<AsyncWebServerRequest> &w) { return w.expired(); });

  if (_closing) {
    // end() was called while this connection was being accepted: tear it down
    // immediately so it cannot keep holding the listening port.
    if (AsyncClient *c = client->client()) {
      c->abort();
    }
    return;
  }
...
void AsyncWebServer::_teardownClients() {
  asyncsrv::lock_guard_type lock(_clients_mutex);
  _closing = true;
  ...
  for (auto &w : _clients) {
    if (auto r = w.lock()) {
      if (AsyncClient *c = r->client()) {
        // tcp_abort() sends RST: the local port is freed immediately and no
