...
void AsyncServer::begin() {
  ...
  int8_t err;
  {
    ...
    if (_pcb) {
#if SO_REUSE
      // Allow the listening port to be rebound immediately after end():
      // lwIP only permits reuse when BOTH pcbs carry SOF_REUSEADDR, and
      // accepted connections inherit it via SOF_INHERITED, so TIME_WAIT
      // sockets from the previous session no longer cause ERR_USE (-8).
