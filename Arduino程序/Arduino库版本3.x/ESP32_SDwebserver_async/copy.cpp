...
void editTxt(AsyncWebServerRequest *request) {
  String txtPath = request->getParam("txtpath")->value();  //提取文件路径
  String txtContent = request->getParam("con")->value();   //提取要保存的内容
  char flag = 0;
  flag = writeFile(my_fs, (char *)txtPath.c_str(), (char *)txtContent.c_str());
  if (flag) {
    request->send(200, "text/html", "保存成功");
  } else {
    request->send(200, "text/html", "保存失败");
  }
}
