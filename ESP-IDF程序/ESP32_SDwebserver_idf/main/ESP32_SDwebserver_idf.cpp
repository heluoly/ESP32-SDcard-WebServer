...
ESP32_SD卡服务器_异步库
作者：B站 狼尾巴的猫
项目实现文件上传下载、播放视频、模式转换（AP STA AP+STA互相转换）、剪切板功能
...
char webDavState = 0;                 //WebDAV当前状态
char webDavMode = 0;                  //WebDAV模式
int webDavPort = 8080;                //WebDAV端口
...
void serverInfo_Display() {
  ...
  if (isWifiOff) {
    ...
    if (serverDisplayState == MY_SERVER_DP_STATE_AP) {
      if (ssid_hidden) {
        if (webDavState) {
          OLED_ShowString_RAM(0, 2, "Mode: AP*+DAV", 16);
        } else {
      ...
      if (webDavState) {
        OLED_ShowString_RAM(0, 2, "Mode: STA+DAV", 16);
      } else {
      ...
      if (ssid_hidden) {
        if (webDavState) {
          OLED_ShowString_RAM(0, 2, "Mode:AP*+STA+DAV", 16);
        } else {
