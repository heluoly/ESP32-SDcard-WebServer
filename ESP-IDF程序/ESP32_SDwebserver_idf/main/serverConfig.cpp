...
const String htmlAP6 = "<br><br><input type=\"submit\" value=\"提交\" /></form><p><a href=\"/\">返回服务器</a> <a href=\"/pageConfigAutoConnect\">配置WiFi</a> <a href=\"/pageConfigWebDav\">配置WebDAV</a></p></div></body></html>";
...
const String htmlAutoConnect6 = "<br><br><input type=\"submit\" value=\"提交\" /></form><p><a href=\"/\">返回服务器</a> <a href=\"/pageConfigAP\">配置热点</a> <a href=\"/pageConfigWebDav\">配置WebDAV</a></p></div></body></html>";
...
const String htmlWebDav1 = "<!DOCTYPE html><html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" /><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\" /><meta http-equiv=\"X-UA-Compatible\" content=\"ie=edge\" /><title>配置WebDAV</title><style>.container {width: 320px;margin: 0 auto;}</style></head><body><div class=\"container\">";
const String htmlWebDav2 = "<h2>配置WebDAV</h2><form action=\"/configWebDav\">WebDAV端口： <input name=\"webDavPort\" type=\"text\" value=\"";
const String htmlWebDav3 = "\" oninput=\"value=value.replace(/\\D/g, '')\" required=\"required\" maxlength=\"5\" placeholder=\"0 - 65535\" /><br>WebDAV模式： <input name=\"webDavMode\" type=\"radio\" value=\"0\"";
const String htmlWebDav4 = "<br><br><input type=\"submit\" value=\"提交\" /></form><p><a href=\"/\">返回服务器</a> <a href=\"/pageConfigAP\">配置热点</a> <a href=\"/pageConfigAutoConnect\">配置WiFi</a></p></div></body></html>";
...
void configAP(AsyncWebServerRequest *request) {
  ...
  flag = flag + configWriteOpen(config_fs, "/config.txt", filetxt);
  ...
  flag = flag + configWriteClose(config_fs, "/config.txt", (char *)filetxt);
...
void pageConfigWebDav(AsyncWebServerRequest *request) {
  ...
  message += htmlWebDav3;
  if (webDavMode) {
    message += ">关闭<input name=\"webDavMode\" type=\"radio\" value=\"1\" checked>开启";
  } else {
    message += " checked>关闭<input name=\"webDavMode\" type=\"radio\" value=\"1\">开启";
  }
  message += htmlWebDav4;
  request->send(200, "text/html", message);
}
...
void configWebDav(AsyncWebServerRequest *request) {
  ...
  if (webDavPort3 < 0 || webDavPort3 > 65535) {
    message += htmlWebDav1 + "<h2>WebDAV端口异常</h2>" + htmlfoot;
    request->send(200, "text/html", message);
    ...
    char flag = 0;
    flag = flag + configWriteOpen(config_fs, "/config.txt", filetxt);
    flag = flag + configRewrite("webDavPort", (char *)webDavPort2.c_str(), (char *)filetxt);
    flag = flag + configRewrite("webDavMode", (char *)webDavMode2.c_str(), (char *)filetxt);
    flag = flag + configWriteClose(config_fs, "/config.txt", (char *)filetxt);

    if (flag == 4) {
      webDavPort = webDavPort3;
      webDavMode = String2Char((char *)webDavMode2.c_str());
      message += htmlWebDav1 + "<h2>配置成功</h2>" + htmlfoot;
      request->send(200, "text/html", message);
    } else {
      message += htmlWebDav1 + "<h2>配置失败</h2>" + htmlfoot;
      request->send(200, "text/html", message);
    }
  }
}
